...
@APP.middleware("http")
async def free_cuda_cache_middleware(request, call_next):
...
async def tts_handle(req: dict):
    ...
    req["batch_size"] = max(1, min(8, int(req.get("batch_size", 4))))
    ...
    print(f"[tts] text_len={len(str(req.get('text','')))} streaming={streaming_mode} media={media_type} ref={req.get('ref_audio_path')}", flush=True)
    ...
    try:
        ...
        if streaming_mode:
            ...
            def streaming_generator(tts_generator: Generator, media_type: str):
                try:
                    ...
                    traceback.print_exc()
                    print("[tts] streaming generation failed, closing stream", flush=True)
        ...
        traceback.print_exc()
        return JSONResponse(status_code=500, content={"message": "tts failed", "Exception": "%s: %s" % (type(e).__name__, e)})
